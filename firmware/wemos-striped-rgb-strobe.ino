...
const uint8_t strips[4] = {STRIP1_LEDS, STRIP2_LEDS, STRIP3_LEDS, STRIP4_LEDS};

// WiFi & Web Server
char ssid[32] = "STRIPED-RGB-STROBE";
char password[32] = "admin123";
char webPassword[32] = "admin123";
...
void loop() {
  ...
  for (int i = 0; i < MAX_SCHEDULES; i++) {
    uint16_t schedMode;
    uint8_t schedBrightness, schedSpeed, schedColor;
    if (scheduler.isScheduleActive(i, schedMode, schedBrightness, schedSpeed, schedColor)) {
      currentMode = schedMode;
      brightness = schedBrightness;
      speed = schedSpeed;
      selectedColor = schedColor;
      FastLED.setBrightness(brightness);
      break;
    }
  }
...
void setupWebServer() {
  ...
  server.on("/api/schedule/delete", handleDeleteSchedule);
...
void handleWiFiSettings() {
  ...
  if (server.hasArg("password")) {
    strncpy(password, server.arg("password").c_str(), 31);
  }
  if (server.hasArg("webPassword")) {
    strncpy(webPassword, server.arg("webPassword").c_str(), 31);
  }
  
  saveSettingsToFS();
  server.send(200, "text/plain", "WiFi settings updated. Restart device to apply.");
}
...
void handleListSchedules() {
  ...
  for (int i = 0; i < MAX_SCHEDULES; i++) {
    if (scheduler.schedules[i].enabled) {
      JsonObject sched = schedArray.createNestedObject();
      sched["index"] = i;
      sched["name"] = scheduler.schedules[i].name;
      sched["mode"] = scheduler.schedules[i].mode;
      sched["brightness"] = scheduler.schedules[i].brightness;
      sched["speed"] = scheduler.schedules[i].speed;
      sched["color"] = scheduler.schedules[i].color;
      sched["startHour"] = scheduler.schedules[i].startHour;
      sched["startMinute"] = scheduler.schedules[i].startMinute;
      sched["endHour"] = scheduler.schedules[i].endHour;
      sched["endMinute"] = scheduler.schedules[i].endMinute;
      sched["daysOfWeek"] = scheduler.schedules[i].daysOfWeek;
    }
  }
...
void loadSettingsFromFS() {
  if (LittleFS.exists("/settings.json")) {
    ...
    if (file) {
      ...
      strncpy(ssid, doc["ssid"], 31);
      strncpy(password, doc["password"], 31);
      strncpy(webPassword, doc["webPassword"], 31);
      return;
    }
  }
}
...
<body>
  ...
  <script>
    const pass = new URLSearchParams(window.location.search).get('pass') || 'admin123';
    let animating = true;
    ...
    function updateControl() {
      ...
      fetch('/api/control?mode=' + mode + '&brightness=' + brightness + '&speed=' + speed + '&color=' + color + '&pass=' + pass)
        .then(r => r.text())
    ...
    function loadPreset() {
      fetch('/api/preset/load?index=0&pass=' + pass)
        .then(r => r.text())
        .then(d => {
...
String getLoginPage() {
  return String(LOGIN_PAGE);
}

String getControlPage() {
  return String(CONTROL_PAGE);
}
